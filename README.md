...
│   ├── app/
│   │   ├── api/v1/       # REST-Endpunkte (19 Router-Module)
...
│   │   │   ├── ingestion/    # Datenpipelines & Clients (10 Quellen)
...
- **Attack Path Graph:** Per-CVE Mermaid-Visualisierung der Angriffskette (`entry → asset → package → CVE → CWE → CAPEC → exploit → impact → fix`) mit Likelihood-, Exploit-Maturity-, Reachability- und Business-Impact-Labels. Strukturlayer ist deterministisch aus EPSS / KEV / CWE / CAPEC / Inventory abgeleitet (keine AI-Halluzination); optionaler AI-generierter Prosa-Narrative liegt darüber. Eingebettet als neuer Tab auf der Vulnerability-Detail-Seite und als Inline-Expansion in den Scan-Findings. Per MCP-Tool `refine_attack_path_analysis` interaktiv unter Hypothesen erkundbar (`reachability`, `entry_point`, `network_exposure`, …).
- **Cross-CVE Attack Chain:** Pro-Scan Mehrstufen-Angreiferketten-Visualisierung als top-level Tab auf der Scan-Detail-Seite. Synthesisiert *alle* Findings eines Scans in eine ATT&CK-Kill-Chain-Erzählung (Foothold → Credential Access → Privilege Escalation → Lateral Movement → Impact). Stages werden deterministisch nach CWE→ATT&CK-Stage-Map gebucketet; pro Stufe top-5 CVEs nach CVSS und top-2 CAPEC-Patterns. Optionale AI-Narrative (≤350 Wörter) liefert prosaische Story plus Patch-Priorisierung ("welcher CVE zuerst patchen, um die Kette zu brechen?"). Persistiert in MongoDB; via MCP `prepare_/save_scan_attack_chain_analysis` auch ohne Web-UI nutzbar.
...
- `GET /api/v1/scans/{scan_id}/attack-chain` — Deterministischer Cross-CVE-Chain-Graph für einen SCA-Scan (Findings gebucketet nach ATT&CK-Stufen) plus persistierter AI-Narrative. Optionaler `?language=` Param. **Nicht** AI-Password-gegated.
- `POST /api/v1/scans/{scan_id}/attack-chain` — Erzeugt eine AI-Narrative-Beschreibung der verketteten Angriffskette (HTTP 202, Background-Task; Frontend pollt den Scan bis `attack_chain` befüllt ist — kein SSE). Persistiert als `attack_chain` / `attack_chains[]` auf dem Scan-Dokument in MongoDB.
...
- `GET /api/v1/scans/findings` — Consolidated Findings über alle aktuellen Scans (Root-Tab)
- `GET /api/v1/scans/sbom` — Consolidated SBOM über alle aktuellen Scans
...
- **35 Tools** (Server-Name: `hecate`):
  ...
  - Write (Quell-IP bei Authorize in `MCP_WRITE_IP_SAFELIST`): `trigger_scan`, `trigger_sync`, `save_vulnerability_ai_analysis`, `save_vulnerabilities_ai_batch_analysis`, `save_scan_ai_analysis`, `save_attack_path_analysis`, `save_scan_attack_chain_analysis`
...
poetry run python -m app.cli enrich-mal [--limit N]              # deps.dev-Anreicherung bestehender MAL-*-Dokumente (broad >=0 → echte Versionen)
